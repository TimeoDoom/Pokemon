...
$(document).ready(function () {
  ...
  const ptsAtcksBase = $(
    'tr > td:contains("Points d\'attaque de base")',
  ).last();
  ...
  function displayTablePage(
    ...
    if (nameVal != null) {
      pagePokemon = pagePokemon.filter((p) =>
        p.name.toLowerCase().includes(nameVal.toLowerCase()),
      );
    }
    ...
    if (triTypes == 1) {
      pagePokemon.sort((p1, p2) =>
        p1.types[0].name.localeCompare(p2.types[0].name),
      );
    } else if (triTypes == 2) {
      pagePokemon.sort((p1, p2) =>
        p2.types[0].name.localeCompare(p1.types[0].name),
      );
    }
    ...
    switch (triPtsDefBase) {
      case 0:
        ...
        ptsDefBase
          ...
          .text("Points de défense de base 🔼");
        break;
      case 1:
        triPtsDefBase = 2;
        ptsDefBase
